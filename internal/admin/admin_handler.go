...
func NewAdminHandler(queries *repository.Queries, authService *service.AuthService, tokenService *service.TokenService) *AdminHandler {
	return &AdminHandler{queries: queries, authService: authService, tokenService: tokenService, fs: templateFS}
}
...
func (h *AdminHandler) Pricing(c *gin.Context) {
	...
	// List all plans (no product filter)
	plans, _ := h.queries.ListPricingPlansByProduct(ctx, pgtype.Text{Valid: false})
